...
const QUEST_MODEL = 'claude-opus-4-8';
...
async function pickWordsForUser(supabase, userId, count) {
  const { data: words, error: wordsError } = await supabase
    .from('words')
    .select('id, term, definition, example_sentence, part_of_speech');
...
const QUEST_SCHEMA = {
  type: 'object',
  properties: {
    title: { type: 'string', description: 'A short, fun title for this side quest.' },
    setting: { type: 'string', description: 'One sentence setting the scene for the conversation.' },
    lines: {
      type: 'array',
      items: {
        type: 'object',
    ...
    quiz: {
      ...
      items: {
        type: 'object',
        properties: {
          word: { type: 'string', description: 'The exact vocabulary word this question tests, as given.' },
          question: { type: 'string' },
          options: { type: 'array', items: { type: 'string' }, minItems: 4, maxItems: 4 },
          correctIndex: { type: 'integer', description: 'Index (0-3) of the correct option.' }
        },
        required: ['word', 'question', 'options', 'correctIndex'],
        additionalProperties: false
      }
    }
  },
  required: ['title', 'setting', 'lines', 'quiz'],
  additionalProperties: false
};
...
// Calls Claude to write the quest story + quiz for the given words (max 5).
async function generateSideQuestStory(words) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 2000,
    system:
      'You write short, fun, level-appropriate conversational stories ("side quests") for a language-learning app. Follow the requested JSON schema exactly.',
    messages: [{ role: 'user', content: buildQuestPrompt(words) }],
