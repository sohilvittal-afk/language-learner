...
- **Word Bank** (`words.html`, `GET/POST/DELETE /api/words`) is the shared vocabulary every learner draws from. Anyone logged in can browse it; admins and super admins can add or remove words (term, definition, example sentence, part of speech, difficulty, and **which language the word is in** — English, Dutch, German, etc.; the list lives in `lib/translation.js`).
