...
async function kimiChatJson({ system, user, maxTokens = 2500 }) {
  ...
  while (true) {
    try {
      ...
      // budget sized for the answer alone can come back empty. Try once
      // more with a much larger budget before giving up.
      if (!tokenBumpTried && err.emptyContent && err.finishReason === 'length') {
        tokenBumpTried = true;
